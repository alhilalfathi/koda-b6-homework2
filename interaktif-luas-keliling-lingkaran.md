...
flowchart TD
    mulai@{ shape: circle}
    outputAwal@{ shape: lean-r, label: "Output: #quot;Masukan nilai jari-jari#quot;"}
    input@{ shape: lean-r, label: "Input: rString"}
    numberTostring@{ label: "const r = Number(rString)"}
    letphi@{ label: "phi = 0"}
    ...
    mulai-->outputAwal-->input-->numberTostring-->letphi-->if
    if--false-->phi1
    if--true-->phi2
    phi2-->rumus
    phi1-->rumus
